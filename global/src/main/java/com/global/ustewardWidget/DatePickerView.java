...
public class DatePickerView extends View {
    ...
    private void init() {
        ...
        mPaint.setColor(ContextCompat.getColor(context, R.color.text_black));
        //第二个paint
        nPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        nPaint.setStyle(Style.FILL);
        nPaint.setTextAlign(Align.CENTER);
        nPaint.setColor(ContextCompat.getColor(context, R.color.text_gray));
    }
    ...
    static class UpdateHandler extends Handler {
        WeakReference<DatePickerView> mView;

        UpdateHandler(DatePickerView view) {
            mView = new WeakReference<>(view);
        }

        @Override
        public void handleMessage(Message msg) {
            if (Math.abs(mView.get().mMoveLen) < SPEED) {
                mView.get().mMoveLen = 0;
                if (mView.get().mTask != null) {
                    mView.get().mTask.cancel();
                    mView.get().mTask = null;
                    mView.get().performSelect();
                }
            } else {
                // 这里mMoveLen / Math.abs(mMoveLen)是为了保有mMoveLen的正负号，以实现上滚或下滚
                mView.get().mMoveLen = mView.get().mMoveLen - mView.get().mMoveLen / Math.abs(mView.get().mMoveLen) * SPEED;
            }
